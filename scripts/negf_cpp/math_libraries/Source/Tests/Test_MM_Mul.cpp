...
void Test_MM_Mul::Generate_Answer()
{
    const auto& h_A = h_A_data.const_table();
    const auto& h_B = h_B_data.const_table();
...
void Test_MM_Mul::Print_Answer() { Print_Table2D(h_ANS_data, "ANS = A * B"); }

void Test_MM_Mul::Copy_Soln_To_Host()
{
...
void Test_MM_Mul::Verify(bool flag_print_answer)
{
    if (!output_copied_to_host) Copy_Soln_To_Host();
