...
void MathLib::ConjugateTranspose(ComplexType* d_AconjT,
                                 const ComplexType* d_A,
                                 unsigned int A_rows,
                                 unsigned int A_cols) 
{
    ...
    cublasStatus_t status = cublasZgeam(handle, CUBLAS_OP_C, CUBLAS_OP_N,                   
                                        A_cols, A_rows, &alpha,                     
                                        reinterpret_cast<const cuDoubleComplex*>(d_A),  
                                        A_rows, &beta,                          
                                        reinterpret_cast<const cuDoubleComplex*>(d_A),  
                                        A_rows,    
                                        reinterpret_cast<cuDoubleComplex*>(d_AconjT),  
                                        A_cols);    
    checkCudaErrors(status);
                                            ...
                                            unsigned int A_cols) 
{
    ...
    //// Query workspace size for eigendecomposition of a complex Hermitian matrix
    ...
    //// Allocate workspace
    //ComplexType* d_work;
    //checkCudaErrors(cudaMalloc((void**)&d_work, work_size * sizeof(ComplexType)));
