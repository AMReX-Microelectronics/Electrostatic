 ...
 */
std::vector<char> get_serialized_gather_rank_msgs(
    const std::vector<Msg> &my_msgs, const int gather_rank, const int my_rank);

/**
 * \brief This function generates data to send back to the "gather rank"
 *
 * @param[in] serialized_gather_rank_msgs the serialized messages of the gather
 * rank
 ...
 * @param[in] is_gather_rank true if the rank is the "gather rank", false
 * otherwise
 * @return a byte array to send back to the "gather rank" (or a dummy vector in
 * case is_gather_rank is true)
 */
std::vector<char> compute_package_for_gather_rank(
    const std::vector<char> &serialized_gather_rank_msgs,
    const std::int64_t gather_rank_how_many_msgs,
    const std::map<Msg, std::int64_t> &my_msg_map, const bool is_gather_rank);

/**
 * \brief This collective function gathers data generated with
 * compute_package_for_gather_rank to the gather rank. If my_rank != gather_rank
 * the function returns dummy data. Otherwise the function returns a pair
 * containing: 1) a byte array containing info on messages seen by other ranks
 * 2) a vector of displacements to access data corresponding to a given rank
 *
 * @param[in] package_for_gather_rank a byte array generated by
 * compute_package_for_gather_rank
 * @param[in] gather_rank the ID of the "gather rank"
 * @param[in] my_rank the ID of the current rank
 * @return (see function description)
 */
std::pair<std::vector<char>, std::vector<int>> gather_all_data(
    const std::vector<char> &package_for_gather_rank, const int gather_rank,
    const int my_rank);
 ...
 * @return a byte array
 */
std::vector<char> serialize_msgs(const std::vector<Msg> &msgs);
 ...
 */
std::vector<Msg> deserialize_msgs(const std::vector<char> &serialized);
}  // namespace
#endif
...
Priority Utils::MsgLogger::StringToPriority(const std::string &priority_string)
{
    if (priority_string == "high")
        ...
        amrex::Abort("Priority string '" + priority_string +
                     "' not recognized");

    // this silences a "non-void function does not return a value in all control
    // paths" warning
    return Priority::low;
}
...
MsgWithCounter MsgWithCounter::deserialize(
...
std::vector<char> MsgWithCounterAndRanks::serialize() const
{
    ...
    put_in_vec(this->msg_with_counter.serialize(),
               serialized_msg_with_counter_and_ranks);
...
MsgWithCounterAndRanks MsgWithCounterAndRanks::deserialize(
    std::vector<char>::const_iterator &it)
{
    ...
    const auto vec = get_out_vec<char>(it);
    auto iit = vec.begin();
    msg_with_counter_and_ranks.msg_with_counter =
        MsgWithCounter::deserialize(iit);
...
void Logger::record_msg(Msg msg) { m_messages[msg]++; }
...
Logger::collective_gather_msgs_with_counter_and_ranks() const
{
    ...
    if (m_num_procs == 1) return one_rank_gather_msgs_with_counter_and_ranks();
    ...
    const auto package_for_gather_rank =
        ::compute_package_for_gather_rank(serialized_gather_rank_msgs,
                                          gather_rank_how_many_msgs, m_messages,
                                          is_gather_rank);

    // Send back all the data to the "gather rank"
    const auto [all_data, displacements] =
        ::gather_all_data(package_for_gather_rank, gather_rank, m_rank);

    // Use the gathered data to generate (on the "gather rank") a vector of all
    // the messages seen by all the ranks with the corresponding counters and
    // emitting rank lists.
    auto msgs_with_counter_and_ranks =
        compute_msgs_with_counter_and_ranks(m_messages, all_data, displacements,
                                            gather_rank);

    // If the current rank is not the I/O rank, send msgs_with_counter_and_ranks
    // to the I/O rank
    swap_with_io_rank(msgs_with_counter_and_ranks, gather_rank);
...
Logger::one_rank_gather_msgs_with_counter_and_ranks() const
{
    ...
    {
        res.emplace_back(
            MsgWithCounterAndRanks{MsgWithCounter{el.first, el.second}, true,
...
std::pair<int, int> Logger::find_gather_rank_and_its_msgs(
    int how_many_msgs) const
{
    ...
    if (m_am_i_io)
    {
        ...
        max_rank =
            (max_items == how_many_msgs) ? m_io_rank : it_max - num_msg.begin();
    }
...
std::vector<MsgWithCounterAndRanks> Logger::compute_msgs_with_counter_and_ranks(
    const std::map<Msg, std::int64_t> &my_msg_map,
    const std::vector<char> &all_data, const std::vector<int> &displacements,
    const int gather_rank) const
{
    ...
    for (const auto &el : my_msg_map)
    {
        msgs_with_counter_and_ranks.emplace_back(
            MsgWithCounterAndRanks{MsgWithCounter{el.first, el.second}, false,
                                   std::vector<int>{m_rank}});
    }
...
#pragma omp critical
        ...
        for (int i = 0; i < how_many_additional_msgs_with_counter; ++i)
        {
            ...
            auto msg_with_counter = MsgWithCounter::deserialize(
                serialized_msg_with_counter.begin());
...
#pragma omp critical
#endif
            {
                if (tmap.find(msg_with_counter.msg) == tmap.end())
                {
                    const auto msg_with_counter_and_ranks =
                        MsgWithCounterAndRanks{msg_with_counter, false,
                                               std::vector<int>{rr}};
    ...
    // Check if messages emitted by "gather rank" are actually emitted by all
    // ranks
    ...
    int gather_rank) const
{
    if (gather_rank != m_io_rank)
    {
        if (m_rank == gather_rank)
        {
            ...
            int list_size =
                static_cast<int>(msgs_with_counter_and_ranks.size());
    ...
    const std::vector<Msg> &my_msgs, const int gather_rank, const int my_rank)
{
    ...
    if (is_gather_rank)
    {
        ...
        size_serialized_gather_rank_msgs =
            static_cast<int>(serialized_gather_rank_msgs.size());
    }

    amrex::ParallelDescriptor::Bcast(&size_serialized_gather_rank_msgs, 1,
                                     gather_rank);

    if (!is_gather_rank)
        serialized_gather_rank_msgs.resize(size_serialized_gather_rank_msgs);

    amrex::ParallelDescriptor::Bcast(serialized_gather_rank_msgs.data(),
                                     size_serialized_gather_rank_msgs,
                                     gather_rank);
    ...
    const std::map<Msg, std::int64_t> &my_msg_map, const bool is_gather_rank)
{
    if (!is_gather_rank)
    {
        ...
        std::vector<std::int64_t> gather_rank_msg_counters(
        ...
        for (const auto &el : msgs_to_send)
            put_in_vec<char>(MsgWithCounter{el.first, el.second}.serialize(),
                             package);
    ...
    const int my_rank)
{
    ...
    {
        amrex::ParallelDescriptor::Gather(static_cast<int>(
                                              package_for_gather_rank.size()),
                                          gather_rank);
        amrex::ParallelDescriptor::Gatherv(package_for_gather_rank.data(),
                                           package_for_gather_rank.size(),
                                           all_data.data(), package_lengths,
                                           displacements, gather_rank);
    }
    else
    {
        ...
        // ...). Rotating this last vector by one is just shifting:
        // (n1+n2+n3+n4+...,n1, n1+n2, n1+n2+n3, ...). Then we just need to
        // replace the first element with zero: (0,n1, n1+n2, n1+n2+n3, ...).
        ...
        std::rotate(displacements.rbegin(), displacements.rbegin() + 1,
                    displacements.rend());
        displacements[0] = 0;

        all_data.resize(total_sum);

        amrex::ParallelDescriptor::Gatherv(static_cast<char *>(nullptr), 0,
                                           all_data.data(), package_lengths,
                                           displacements, gather_rank);
    }
