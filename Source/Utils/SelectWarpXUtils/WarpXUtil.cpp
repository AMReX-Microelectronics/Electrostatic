...
void PreparseAMReXInputIntArray(amrex::ParmParse &a_pp,
                                char const *const input_str, const bool replace)
{
...
void ParseGeometryInput()
{
    ...
    // Parse amr input, evaluating any expressions since amr does not parse its
    // input
    ParmParse pp_amr("amr");

    // Note that n_cell is replaced so that only the parsed version is written
    // out to the warpx_job_info file. This must be done since yt expects to be
    // able to parse the value of n_cell from that file. For the rest, this
    // doesn't matter.
...
{
    // Since queryWithParser recursively calls this routine, keep track of
    // symbols in case an infinite recursion is found (a symbol's value
    // depending on itself).
    ...
    static std::map<std::string, amrex::Real> warpx_constants = {
        {"clight", PhysConst::c}, {"epsilon0", PhysConst::ep0},
        {"mu0", PhysConst::mu0},  {"q_e", PhysConst::q_e},
        {"m_e", PhysConst::m_e},  {"m_p", PhysConst::m_p},
        {"m_u", PhysConst::m_u},  {"kb", PhysConst::kb},
        {"pi", MathConst::pi},
    };

    for (auto it = symbols.begin(); it != symbols.end();)
    {
        // Always parsing in double precision avoids potential overflows that
        // may occur when parsing user's expressions because of the limited
        // range of exponentials in single precision
        double v;
    ...
    //        amrex::Abort(Utils::TextMsg::Err("makeParser::Unknown symbol
    //        "+s));
    //    }
    return parser;
}
...
// Overloads for float/double instead of amrex::Real to allow makeParser() to
// query for my_constants as double even in single precision mode Always parsing
// in double precision avoids potential overflows that may occur when parsing
// user's expressions because of the limited range of exponentials in single
// precision
                    ...
                    float &val)
{
    ...
    if (is_specified)
    {
        // If so, create a parser object and apply it to the value provided by
        // the user.
        std::string str_val;
...
void getWithParser(const amrex::ParmParse &a_pp, char const *const str,
                   float &val)
{
    // If so, create a parser object and apply it to the value provided by the
    // user.
    std::string str_val;
    Store_parserString(a_pp, str, str_val);
    val = static_cast<float>(parseStringtoReal(str_val));
...
int queryArrWithParser(const amrex::ParmParse &a_pp, char const *const str,
                       std::vector<amrex::Real> &val, const int start_ix,
                       const int num_val)
{
    // call amrex::ParmParse::query, check if the user specified str.
    ...
    if (is_specified)
    {
        // If so, create parser objects and apply them to the values provided by
        // the user.
        ...
        {
            val[i] =
                static_cast<amrex::Real>(parseStringtoReal(tmp_str_arr[i]));
        }
    }
    // return the same output as amrex::ParmParse::query
    return is_specified;
...
int queryArrWithParser(const amrex::ParmParse &a_pp, char const *const str,
                       std::vector<int> &val, const int start_ix,
                       const int num_val)
{
...
int safeCastToInt(const amrex::Real x, const std::string &real_name)
{
    ...
    // (2.0*(numeric_limits<int>::max()/2+1)) converts
    // numeric_limits<int>::max()+1 to a real ensuring accuracy to all digits
    // This accepts x = 2**31-1 but rejects 2**31.
    using namespace amrex::literals;
    constexpr amrex::Real max_range =
        (2.0_rt *
         static_cast<amrex::Real>(std::numeric_limits<int>::max() / 2 + 1));
    if (x < max_range)
    {
        ...
        {
            error_detected = true;
            assert_msg = "Negative overflow detected when casting " +
