...
int SignalHandling::parseSignalNameToNumber(const std::string &str)
{
...
void SignalHandling::InitSignalHandling()
{
#if defined(__linux__) || defined(__APPLE__)
    struct sigaction sa;
    ...
    for (int signal_number = 0; signal_number < NUM_SIGNALS; ++signal_number)
    {
        ...
        for (int signal_request = 0; signal_request < SIGNAL_REQUESTS_SIZE;
             ++signal_request)
        {
            signal_active |=
                signal_conf_requests[signal_request][signal_number];
        }
        if (signal_active)
        {
...
void SignalHandling::CheckSignals()
{
    ...
    if (amrex::ParallelDescriptor::MyProc() == 0)
    {
        for (int signal_number = 0; signal_number < NUM_SIGNALS;
             ++signal_number)
        {
            ...
            bool signal_received =
                signal_received_flags[signal_number].exchange(false);

            if (signal_received)
            {
                for (int signal_request = 0;
                     signal_request < SIGNAL_REQUESTS_SIZE; ++signal_request)
                {
                    signal_actions_requested[signal_request] |=
                        signal_conf_requests[signal_request][signal_number];
                }
            }
        }
    ...
    // Due to a bug in Cray's MPICH 8.1.13 implementation (CUDA builds on
    // Perlmutter@NERSC in 2022), we cannot use the MPI_CXX_BOOL C++ datatype
    // here. See WarpX PR #3029 and NERSC INC0183281
    static_assert(sizeof(bool) == 1,
                  "We communicate bools as 1 byte-sized type in MPI");
    BL_MPI_REQUIRE(MPI_Ibcast(signal_actions_requested, SIGNAL_REQUESTS_SIZE,
                              MPI_BYTE, 0, comm, &signal_mpi_ibcast_request));
#endif
}

void SignalHandling::WaitSignals()
{
    // Is any signal handling action configured?
    // If not, we can skip all handling and the MPI communication as well.
    if (!m_any_signal_action_active) return;
...
bool SignalHandling::TestAndResetActionRequestFlag(int action_to_test)
{
...
void SignalHandling::SignalSetFlag(int signal_number)
{
