...
void
c_ChargeDensitySource<ParticleContainerType>::Deposit(amrex::MultiFab* const p_rho_mf)
{   
              ...
              ++pti)
    {
        ...
        {   
            amrex::Real qp = p_occupation[p]*p_charge_unit[p]*unit_charge/vol;
...
template<typename ParticleContainerType>
void
c_ChargeDensitySource<ParticleContainerType>::Gather(amrex::MultiFab* const p_potential_mf)
{   
    auto& rCode  = c_Code::GetInstance();
    auto& rGprop = rCode.get_GeometryProperties();
              ...
              ++pti)
    {
        ...
        amrex::ParallelFor (np, [=] AMREX_GPU_DEVICE (int p)
        {   
            p_par_potential[p] = CloudInCell::Gather_Trilinear(p_par[p].pos(), plo, dx, phi);
        });
    }
}
