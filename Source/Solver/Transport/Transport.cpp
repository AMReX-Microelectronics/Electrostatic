...
void c_TransportSolver::Read_SelfConsistencyInput(amrex::ParmParse &pp)
{
    ...
    amrex::Print() << "##### flag_initialize_inverse_jacobian: "
                   << flag_initialize_inverse_jacobian << "\n";

    if (flag_initialize_inverse_jacobian) Read_InverseJacobianFilename(pp);
}

void c_TransportSolver::Read_InverseJacobianFilename(amrex::ParmParse &pp)
{
...
void c_TransportSolver::Read_DOSInput(amrex::ParmParse &pp)
{
...
void c_TransportSolver::Read_GateTerminalType(amrex::ParmParse &pp)
{
    pp.query("gate_terminal_type", gate_terminal_type_str);
}

void c_TransportSolver::Set_gate_terminal_type(
    const std::string gate_terminal_type_str)
{
    if (doesKeyExist(gate_terminal_type_str))
    {
        amrex::Print() << "##### gate_terminal_type: " << gate_terminal_type_str
                       << "\n";
        gate_terminal_type = c_TransportSolver::map_S_GateTerminalType.at(
...
void c_TransportSolver::Sum_ChargeDepositedByAllNS()
{
    auto &rCode = c_Code::GetInstance();
    auto &rGprop = rCode.get_GeometryProperties();
        ...
        do
        {
            ...
            for (int c = 0; c < vp_CNT.size(); ++c)
            {
                ...
                total_intg_pts_in_this_iter +=
                    vp_CNT[c]->get_Total_Integration_Pts();
            ...
            for (int c = 0; c < vp_CNT.size(); ++c)
            {
                ...
                if (vp_CNT[c]->get_flag_write_at_iter())
                {
                    vp_CNT[c]->Write_InputInducedCharge(
                        vp_CNT[c]->get_iter_filename(), n_curr_in_glo_data);
                }
            ...
            for (int c = 0; c < vp_CNT.size(); ++c)
            {
                if (vp_CNT[c]->get_flag_write_at_iter())
                {
                    ...
                    Write_MoreDataAndComputeCurrent(
                        vp_CNT[c], vp_CNT[c]->get_iter_filename(),
                        compute_current);
                }

                if (flag_write_LDOS_iter and
                    (max_iter + 1) % write_LDOS_iter_period == 0)
                {
                    std::string iter_dos_foldername_str =
                        amrex::Concatenate(vp_CNT[c]->get_iter_foldername() +
                                               "/DOS_iter",
        ...
        if (flag_compute_DOS)
        {
            for (int c = 0; c < vp_CNT.size(); ++c)
            {
                std::string dos_step_foldername_str =
                    amrex::Concatenate(vp_CNT[c]->get_step_foldername() +
                                           "/DOS_step",
                                       step, negf_plt_name_digits);
                ...
                vp_CNT[c]->Compute_DensityOfStates(dos_step_foldername_str,
                                                   flag_write_LDOS);
            }
        }
...
void c_TransportSolver::Set_TerminalBiasesAndContactPotential(NSType const &NS)
{
    auto &rCode = c_Code::GetInstance();
    auto &rGprop = rCode.get_GeometryProperties();
    ...
    if (!NS->is_contact_mu_specified())
    {
        ...
        {
            Vgs =
                rGprop.pEB->Read_SurfSoln(NS->get_Gate_String()) - V_contact[0];
        }
...
template <typename NSType>
void c_TransportSolver::CopyFromNS_ChargeComputedFromNEGF(NSType const &NS)
{
/* (?) Need a strategy to gather data for multiple CNTs*/
/* (?) Future: if condition to check if proc was used for charge computation of
 * this nanostructure*/
/* (?) Future: for multiple nanotubes, then gather the data into a global array
 * and then partition */
...
template <typename NSType>
void c_TransportSolver::CopyToNS_ChargeComputedUsingSelfConsistencyAlgorithm(
    NSType const &NS)
{
    ...
    MPI_Gatherv(&h_n_curr_in(0), NS->MPI_recv_count[my_rank], MPI_DOUBLE,
                &n_curr_in_glo(0), NS->MPI_recv_count.data(),
                NS->MPI_recv_disp.data(), MPI_DOUBLE,
...
template <typename NSType>
void c_TransportSolver::Write_MoreDataAndComputeCurrent(
    NSType const &NS, std::string const &write_filename,
    bool const compute_current_flag)
{
    ...
    {
        NS->Compute_Current();
        NS->Write_Current(m_step, Vds, Vgs, total_intg_pts_in_all_iter,
                          max_iter, Broyden_fraction, Broyden_Scalar);
    }
}

template <typename NSType>
void c_TransportSolver::Create_Global_Output_Data(NSType const &NS)
{
#ifdef BROYDEN_SKIP_GPU_OPTIMIZATION
    ...
    /*offset necessary for multiple NS*/
    MPI_Gatherv(&h_n_curr_out(0), NS->MPI_recv_count[my_rank], MPI_DOUBLE,
                &n_curr_out_glo(0), NS->MPI_recv_count.data(),
                NS->MPI_recv_disp.data(), MPI_DOUBLE,
                ParallelDescriptor::IOProcessorNumber(),
                ParallelDescriptor::Communicator());

    /*offset necessary for multiple NS*/
    MPI_Gatherv(&h_Norm(0), NS->MPI_recv_count[my_rank], MPI_DOUBLE,
                &Norm_glo(0), NS->MPI_recv_count.data(),
                NS->MPI_recv_disp.data(), MPI_DOUBLE,
                ParallelDescriptor::IOProcessorNumber(),
                ParallelDescriptor::Communicator());
...
void c_TransportSolver::Perform_SelfConsistencyAlgorithm()
{
    switch (c_TransportSolver::map_AlgorithmType.at(Algorithm_Type))
    {
        ...
        {
            amrex::Abort(
                "At present, `Broyden's first' algorithm exists with only\
    ...
    amrex::Real const *total_time_counter_diff)
{
    ...
    if (ParallelDescriptor::IOProcessor())
    {
        amrex::Real avg_curr[num_var] = {
            total_max_time_for_current_step[0] / max_iter,
