...
void c_IntegrationPath::Define_GaussLegendrePoints(const ComplexType min,
                                                   ...
                                                   const int id)
{
    ...
    if (id == 0) /*line*/
    {
        ...
        // << "\n"; std::string filename = "GL_Line_Points"; std::ofstream
        // outfile; outfile.open(filename.c_str());
    ...
    {
        amrex::Real E_center_real =
            (max.real() * max.real() - min.real() * min.real() -
             min.imag() * min.imag()) /
            (2 * (max.real() - min.real()));
        ...
        {
            amrex::Real theta_i =
                (theta2 - theta1) * 0.5 * x[i] + (theta2 + theta1) * 0.5;
        ...
        // << "\n"; std::string filename = "GL_Circ_Points"; std::ofstream
        // outfile; outfile.open(filename.c_str());
...
void c_IntegrationPath::Define_RegularPoints(ComplexType min, ComplexType max,
                                             int pts)
{
    E_min = min;
    E_max = max;
