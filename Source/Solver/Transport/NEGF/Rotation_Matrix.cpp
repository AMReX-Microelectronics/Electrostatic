...
c_RotationMatrix::c_RotationMatrix(
    std::unique_ptr<RotationInputParams> rotationParams)
...
void c_RotationMatrix::Define_RotationMatrices()
{
    _rotX[0] = {1, 0, 0};
...
amrex::Vector<amrex::Real> c_RotationMatrix::Get_RotationAnglesInDegrees() const
{
...
void c_RotationMatrix::Print_RotationParams() const
{
    amrex::Print() << "##### read angle type: "
                   << (_p->angle_type == AngleType::Degrees ? "Degrees"
                                                            : "Radians")
                   << "\n";
...
void c_RotationMatrix::Update_RotationMatrices(
    amrex::Vector<amrex::Real> &new_angles)
{
...
void c_RotationMatrix::Rotate(
    amrex::Vector<amrex::Real> &v2, const amrex::Vector<amrex::Real> v1,
...
void c_RotationMatrix::Apply_RotationOrder(amrex::Vector<amrex::Real> &v,
