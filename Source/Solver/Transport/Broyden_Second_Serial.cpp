...
void c_TransportSolver::Execute_Broyden_Modified_Second_Algorithm()
{
    if (ParallelDescriptor::IOProcessor())
    {
        ...
        RealTable1D sum_Fcurr_data({0}, {num_field_sites_all_NS},
                                   The_Pinned_Arena());
        RealTable1D sum_deltaFcurr_data({0}, {num_field_sites_all_NS},
                                        The_Pinned_Arena());
        RealTable1D W_curr_data({0}, {num_field_sites_all_NS},
                                The_Pinned_Arena());
        RealTable1D V_curr_data({0}, {num_field_sites_all_NS},
                                The_Pinned_Arena());
        ...
        //       Restart_Broyden();
        // }
        // else if (Broyden_NormSumIsIncreasing_Step >
        ...
        //    {
        //        Broyden_Scalar =
        //        Broyden_Scalar/std::pow(Broyden_Scalar_Decrease_Factor,Broyden_Correction_Step);
        //        Broyden_Step -= 1;
        //        Broyden_NormSumIsIncreasing_Step = 0;
        //        amrex::Print() << "\n******Reducing Broyden scalar to: " <<
        //        Broyden_Scalar << "\n";
        //    }
        //}
        // else
        ...
        W_Broyden.push_back(
            new RealTable1D({0}, {num_field_sites_all_NS}, The_Pinned_Arena()));
        V_Broyden.push_back(
            new RealTable1D({0}, {num_field_sites_all_NS}, The_Pinned_Arena()));
        ...
        if (m > 0)
        {
            ...
            for (int l = 0; l < num_field_sites_all_NS; ++l)
            {
                ...
                W_curr(l) = -Broyden_fraction * delta_F_curr(l) + delta_n -
                            sum_deltaFcurr(l);
            }
        ...
        for (int l = 0; l < num_field_sites_all_NS; ++l)
        {
            ...
            n_curr_in(l) = n_prev_in(l) -
                           Broyden_Scalar * Broyden_fraction * F_curr(l) -
                           Broyden_Scalar * sum_Fcurr(l);
        }
