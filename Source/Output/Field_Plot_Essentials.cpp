...
void
c_Field_Plot_Essentials::Init_Plot_Field_Essentials(const amrex::Geometry& geom, int flag) 
{
    _geom = &geom;
...
void
c_Field_Plot_Essentials::WriteSingleLevelPlotFile(int step, amrex::Real time, 
                                                  ...
                                                  std::map<std::string,int>& map_all_mf)
{
    ...
    for (auto it : map_all_mf) 
    {
        ...
        if(_output_option[field_number] == 0 or _output_option[field_number] == 1) 
        {
            ...
            amrex::Print() << prt << " copying parameter: " << field_name 
                                  ...
                                  << " _output_option: " << _output_option[field_number] << "\n";
            #endif
    ...
    if(_embedded_boundary_flag) {
        amrex::EB_WriteSingleLevelPlotfile( _plot_file_name, 
                                        *p_all_mf, m_p_name_str, 
                                         *_geom, 
                                         time, step, 
                                         "HyperCLaw-V1.1", _default_level_prefix, "Cell",
                                         _extra_dirs);
   }
#endif
    if(!_embedded_boundary_flag) {
        amrex::WriteSingleLevelPlotfile( _plot_file_name, 
                                        *p_all_mf, m_p_name_str, 
                                         *_geom, 
                                         time, step, 
                                         "HyperCLaw-V1.1", _default_level_prefix, "Cell",
...
void
c_Field_Plot_Essentials::WriteRawFields(const amrex::Vector<amrex::MultiFab*>& m_p_mf,
                                        std::map<std::string,int>& map_all_mf)
{
#ifdef PRINT_NAME
...
int
c_Field_Plot_Essentials::SpecifyOutputOption(amrex::Vector<std::string> fields_to_plot_withGhost_str, 
