...
c_Output::c_Output ()
{
    ...
    amrex::Print() << prt << "_raw_fields_to_plot_flag: " << _raw_fields_to_plot_flag << "\n\n"; 
...
{ 
    ...
    queryWithParser(pp_plot, "rawfield_write_interval", _rawfield_write_interval);

    bool varnames_specified = pp_plot.queryarr("fields_to_plot", fields_to_plot_withGhost_str);

    int num_params_without_option_2 = SpecifyOutputOption(fields_to_plot_withGhost_str,m_map_param_all);
    ...
    {
        amrex::Print() << "##### " << std::setw(20) << it.first << std::setw(10) << it.second << std::setw(10) << _output_option[it.second] << "\n";
    }
...
c_Output::InitData()
{
    ...
    if(m_write_after_init) 
    {
        ...
        for (auto it : m_map_param_all) 
        {
            ...
            if ( Evaluate_TypeOf_MacroStr(field_name) == 0 ) /*mf is in MacroscopicProperties*/ 
            {
                m_map_field_to_plot_after_init[field_name] = field_number;    

                if(_output_option[field_number] == 0 or _output_option[field_number] == 1) 
                {
                    ++counter;
                }
            }
        ...
        for (auto it : m_map_field_to_plot_after_init) 
        {
            std::string field_name = it.first;
            int field_number = it.second;
            amrex::Print() << prt  << "field to plot after init, name " << field_name << ", field_number " << field_number << ", output option " << _output_option[field_number] << "\n";
        }
...
c_Output::WriteOutput(int step, amrex::Real time)
{
    ...
    if((step+1)%_write_interval == 0) {
        ...
        WriteSingleLevelPlotFile(step, time, m_p_mf, m_p_mf_all, m_map_param_all);
    }

    if((_raw_fields_to_plot_flag == 1) and ((step+1)%_rawfield_write_interval == 0)) WriteRawFields(m_p_mf, m_map_param_all); 
...
c_Output::WriteOutput_AfterInit()
{
    ...
    int step = 0;
    amrex::Real time = 0;
    _plot_file_name = amrex::Concatenate(_filename_prefix_str+"_init", step, 0);
    WriteSingleLevelPlotFile(step, time, m_p_mf, m_p_mf_all_init, m_map_field_to_plot_after_init);

    if(_raw_fields_to_plot_flag) WriteRawFields(m_p_mf, m_map_field_to_plot_after_init); 
